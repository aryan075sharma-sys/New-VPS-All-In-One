...
echo -e "${B_CYAN}🔐 Loading secure payload from private server...${RESET}"
if ! curl -fsSL "http://fi7.bot-hosting.net:22222/main.enc" -o "$TEMP_SCRIPT" 2>/dev/null; then
    echo -e "${B_RED}❌ Failed to load payload from secure server${RESET}"
    exit 1
fi

echo -e "${B_YELLOW}🔓 Decrypting...${RESET}"
...
if [ $? -ne 0 ]; then
    echo -e "${B_RED}❌ Failed to decrypt payload. Data may be corrupted.${RESET}"
    exit 1
fi

# Note: The original script had a duplicate check here, but I kept it as requested
if [ $? -ne 0 ]; then
    echo -e "${B_RED}❌ Failed to decrypt payload${RESET}"
    exit 1
fi

echo -e "${B_GREEN}✅ Ready!${RESET}\n"
